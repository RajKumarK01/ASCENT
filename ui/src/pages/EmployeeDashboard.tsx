...
import { StudyCheckinButton } from '../components/StudyCheckinButton'
...
export function EmployeeDashboard() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      checkTodayCompleted(contributionData)
  ...
  function checkTodayCompleted(contribData: ContributionData) {
    const today = new Date()
    const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`
    setTodayCompleted((contribData[dateStr] ?? 0) > 0)
  }
